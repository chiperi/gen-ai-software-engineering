...
with a companion **Angular 22 + Microsoft Fluent** frontend for visual verification. The whole
project was developed **with AI assistance**, following a deliberate workflow: engineer a strong
prompt → write a TDD plan → implement cycle by cycle → verify locally → build the UI → polish.
This README documents both the result and the AI‑assisted process.

---

## 🧭 Backend approaches

The backend was implemented in **four different technologies, all from the same TDD plan**
([`PLAN-BACKEND-TDD.md`](PLAN-BACKEND-TDD.md)) — the same Red → Green → Refactor cycles and the
same HTTP contract on port 3000, so the one Angular frontend runs against any of them unchanged:

| Approach | Stack | Location | Tests | Run |
|---|---|---|---|---|
| **Primary (documented below)** | Java 17 + Spring Boot 3 | [`backend/`](backend) | 53 | `mvn spring-boot:run` |
| Alternative | Go 1.22 + Chi | [`backend-go/`](backend-go) | 50 | `go run ./cmd/server` |
| Alternative | Python + FastAPI | [`backend-fastapi/`](backend-fastapi) | 32 | `uvicorn app.main:app --port 3000` |
| Alternative | C# / .NET 10 Minimal API | [`backend-dotnet/`](backend-dotnet) | 18 | `dotnet run --project src/BankingApi` |

> **This README describes the Java + Spring Boot implementation in detail.** The **Go**,
> **FastAPI**, and **C# / .NET** versions were then generated by following the *same*
> prompt-and-plan workflow — proving the approach is technology-agnostic. Each has its own
> README (e.g. [`backend-go/README.md`](backend-go/README.md),
> [`backend-fastapi/README.md`](backend-fastapi/README.md),
> [`backend-dotnet/README.md`](backend-dotnet/README.md)) and all expose identical endpoints
> on port 3000. Each backend also serves its own API docs (Swagger/Scalar) and
> `/actuator/health`.
...
The simplest way to run everything is the interactive launcher — it lets you **pick a backend,
then a frontend**, starts them together, seeds sample data, opens the app, and stops everything
on `Ctrl+C`:
...
![run.sh — choose a backend, then a frontend](docs/screenshots/21.png)
*`run.sh` prompts for a backend (Java / Go / FastAPI / .NET), then a frontend
(Angular / Electron / Flutter / none), and runs the chosen combination.*

Prefer to run a single backend or frontend by hand? Each project has its own README with the
exact command; see **[HOWTORUN.md](HOWTORUN.md)** for the full rundown.

---

## 📋 Table of contents

- [Backend approaches](#-backend-approaches)
...
> **Bonus — an alternative backend in Go.** The same API is also implemented in **Go + Chi**
> under [`backend-go/`](backend-go), built from the identical 9‑cycle TDD plan (50 tests, incl.
> health + Scalar docs). It serves the same contract on port 3000, so the Angular frontend works
> against it unchanged. Run it with `./run.sh` (choose Go) or `cd backend-go && go run ./cmd/server`.
> See [Backend approaches](#-backend-approaches).
...
Finally I added an interactive **launcher** (`run.sh`) that lets you pick a backend and a
frontend, then runs them, seeds
...
See [Quick start](#-quick-start) above for `./run.sh`. Full manual instructions — prerequisites,
per-backend commands, tests, endpoints, health and docs URLs — are in **[HOWTORUN.md](HOWTORUN.md)**.
...
├── run.sh                  # interactive launcher: pick a backend + a frontend
├── demo/                   # run.sh, seed.sh, sample-requests.http, sample-data.json
├── docs/screenshots/       # screenshots for this README
├── backend/                # PRIMARY: Spring Boot API (src, pom.xml, tests.json)
├── backend-go/             # ALTERNATIVE: Go + Chi API, same plan
├── backend-fastapi/        # ALTERNATIVE: Python + FastAPI API, same plan
├── backend-dotnet/         # ALTERNATIVE: C# / .NET 8 Minimal API, same plan
└── frontend/               # Angular 22 + Fluent UI (works with any backend)
```
